...
def comprehensive_solution_checker():
    ...
    # Extract student answer from the solution later
    student_answer = ""
    
    print("\n📖 STUDENT'S SOLUTION")
    print("─" * 50)
    print("💡 Please type your complete solution:")
    print("💡 Type each step and press Enter after each one.")
    print("💡 When you're completely done, press Enter twice.")
    
    # Get the complete solution - unlimited input until double Enter
    student_solution = []
    ...
    try:
        ...
        # Show verification status - now more dynamic
        if student_mentions_verification or student_shows_verification_work:
            if student_shows_verification_work:
                print("✅ Verification: (MENTIONED and SHOWN by student)")
            else:
                print("✅ Verification: (MENTIONED but NOT SHOWN by student)")
        else:
            print("✅ Verification: (NOT MENTIONED by student)")
        
        # Add explanation of verification status
        if student_shows_verification_work and not student_mentions_verification:
            print("   💡 We detected verification work even without explicit keywords!")
        
        # Quick verification summary
        verification_score, _ = analyze_verification_context(student_solution, student_answer)
        if verification_score > 0:
            print(f"📊 Verification Quality: {verification_score}/5 points")
            print("   💡 Score based on verification work detected in your solution")
        
        # Advanced verification analysis
        print("\n🔍 ADVANCED VERIFICATION ANALYSIS:")
        print("─" * 40)
        verification_score, verification_details = analyze_verification_context(student_solution, student_answer)
        print(f"📊 Verification Score: {verification_score}/5")
        if verification_details:
            print("📝 Verification Details:")
            for detail in verification_details:
                print(f"   • {detail}")
        else:
            print("📝 No specific verification patterns detected")
    ...
    # Get personalized feedback based on what the student did
    # Treat showing verification work as equivalent to mentioning verification
    effective_verification = student_mentions_verification or student_shows_verification_work
    feedback_lines = get_verification_feedback(effective_verification, student_shows_verification_work)
    for feedback in feedback_lines:
        print(feedback)
    
    # Show specific verification examples based on equation type
    print("\n📚 VERIFICATION EXAMPLES:")
    print("─" * 30)
    examples = get_verification_examples(original_eq)
    for example in examples:
        print(f"   {example}")
    
    # Show detailed analysis of what was detected
    print("\n🔍 DETAILED VERIFICATION ANALYSIS:")
    print("─" * 40)
    detailed_analysis = get_detailed_verification_analysis(student_solution, student_answer)
    for analysis_line in detailed_analysis:
        print(f"   {analysis_line}")
...
def main_menu():
    """
    Main menu for the Rational Equation Solver & Checker
    """
    while True:
        print("\n" + "=" * 70)
        print("                    🧮 RATIONAL EQUATION SOLVER & CHECKER 🧮")
        print("=" * 70)
        print()
        print("📚 Choose an option:")
        print("   1️⃣  Get complete step-by-step solution")
        print("   2️⃣  Check your own solution")
        print("   3️⃣  Exit")
        print()
        ...
        if choice == "1":
            print("\n" + "=" * 70)
            print("                    📖 STEP-BY-STEP SOLUTION MODE 📖")
            print("=" * 70)
            eq = input("📝 Enter a rational equation to solve: ")
            eq = eq.replace('X', 'x')
            ...
            if valid:
                ...
                if isinstance(message, str) and "object is not callable" in message:
                    ...
                    print("Please use '/' for division and '*' for multiplication. For example: 1/(x-2) or 2*x.")
            ...
            print("\n👋 Thank you for using the Rational Equation Solver & Checker!")
            print("Goodbye! 👋")
            break
